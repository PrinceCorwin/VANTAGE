...
### Active Local Database Is Chosen via an External Manifest; Switching Restarts the App
**Rule:** Multiple local SQLite databases can coexist under `%LocalAppData%\VANTAGE`, and which one is active is tracked in an external `databases.json` manifest managed by `Utilities/DatabaseRegistry.cs` — never inside a database. `DatabaseSetup.GetOrSetDatabasePath()` resolves `DbPath` from the manifest's active entry at startup; `InitializeSchemaAtPath(path)` builds a full schema on any file so new databases are created without disturbing the active one. Each entry has a display Name independent of its on-disk filename (`VANTAGE_<slug>.db`), so renaming never moves the file. On first run with no manifest, a `Primary` entry is seeded pointing at the legacy `VANTAGE_Local.db`, so existing installs migrate transparently. Switching the active database writes the manifest and restarts the process; there is no in-session hot-swap. Deleting a database is refused for the active entry.
**Why:** The active-database pointer cannot live in a database because `AppSettings` are stored per-`.db` — every database would carry its own flag. `DbPath` is a startup-set static and views cache 100,000+ rows, so a clean restart is far safer than tearing down and rebuilding every view's data mid-session. Seeding `Primary` at the legacy path means the feature is invisible and lossless for users who never open the manager. Keeping Name separate from filename lets renames be pure metadata edits.
**Date:** August 2026

### User Settings Live in a Shared Per-Machine Database; App Settings Stay Per-Database
**Rule:** `UserSettings` (grid layouts, saved filters, column width/order/visibility, theme, Analysis prefs, UDF header names, tutorial-watch tracking, Project-Dashboard report layouts, etc.) live in a single shared database `%LocalAppData%\VANTAGE\VANTAGE_UserSettings.db`, not inside each project `.db`. `SettingsManager`'s `UserSetting` reads/writes go through `DatabaseSetup.GetUserSettingsConnection()`. `AppSettings` (per-project sync cursors `LastPulledSyncVersion`, etc.) stay inside each project database via `DatabaseSetup.GetConnection()`. On the first launch after the shared store is introduced (detected by the file not yet existing), `SettingsManager.MigrateUserSettingsToSharedStore()` copies the active database's `UserSettings` wholesale into the shared store and unions grid/report layouts from every other registered database (collision-safe rename). Because settings are shared, Reset User Settings is global across all local databases.
**Why:** The Database Manager lets a user keep one project per `.db` and switch between them to keep each `Activities` table small and fast. With `UserSettings` inside each file, switching databases dropped the user's layouts/filters/columns (the new file's settings table was empty). A shared per-machine store makes preferences follow the user across every database with a single source of truth and no drift. `AppSettings` must NOT be shared — its sync cursors are inherently per-database (each database syncs a different set of projects to a different `SyncVersion`), so sharing them would corrupt delta-pull state. `UserSettings` is a flat key-value table with no joins or foreign keys and a single access chokepoint (`SettingsManager`), so relocating it touches only where the connection is opened.
**Date:** August 2026
...
**Rule:** Stub Activities created from the New ActNOs dialog use the literal string `"X"` for the six required-metadata fields P6 cannot supply (`WorkPackage`, `PhaseCode`, `CompType`, `PhaseCategory`, `ROCStep`, `RespParty`). `ProjectID` comes from the dialog's ProjectID picker, `SchedActNO` and `Description` from P6's `task_code` / `task_name`. `PercentEntry`, `ActStart`, `ActFin` mirror P6's `complete_pct`, `act_start_date`, `act_end_date`. `BudgetMHs` mirrors P6's `target_work_qty` but floors at `0.001`; `ClientBudget` is set to `0.001`; `Quantity` is `1`. A stub never carries a zero `BudgetMHs`, `ClientBudget`, or `Quantity`.
**Why:** Required-metadata fields cannot be empty (sync gate blocks empties). A non-empty placeholder is both visible enough to remind the user to come back and fix it AND not blocking — the records can sync, just not earn against an ROC curve until `ROCStep` is real. ROC lookup naturally returns nothing for `X|X|X|X` so EarnMHs stays a function of `PercentEntry × BudgetMHs`, which is the right default. Zero budget/client-budget/quantity are disallowed on any created record (the app applies the same 0.001 minimums on Excel and Takeoff imports); P6 often supplies 0 MHs for these tasks, so the floor is enforced at stub creation rather than trusting the P6 value.
**Date:** June 2026
...
**Superseded 2026-08-19:** MCAA moves to a separate **MCAA Takeoff** module instead of a rate-mode toggle on the shared Takeoff — see "MCAA Takeoff Becomes a Separate Module" below. The Summit-side radios stay until Summit is demoted to material-only.
...
#### MCAA Connection Counting Uses Single-Pass LLM Vision on the PDF, Delivered via Bedrock Batch Inference
**Rule:** The MCAA connection count is produced by a single-pass LLM-vision read of the drawing PDF (Opus 4.8, high effort, with a comprehensive connection-symbol legend + the BOM), returning per-connection type/size/material; stock-length additions are computed deterministically in code. It does NOT come from BOM fitting-end summation, nor from pure computer-vision dot detection. There is no CAD/PCF parse path — client drawings are always PDFs. Production runs go through Amazon Bedrock **Batch Inference** (async, ~24 h turnaround, 50% off on-demand) on the existing AWS account — no paid Anthropic API account. (Implementation pending — full record in `Plans/MCAA-Takeoff/MCAA_Extraction_Design.md`.) **[Batch delivery SUPERSEDED 2026-08-19 → on-demand; the single-pass LLM-vision approach itself stands and is now bench-validated 4/5. See the Aug 19 decisions below.]**
**Why:** Investigated 2026-08-13. BOM-only miscounts (misses pipe-to-pipe, doubles fitting-to-fitting); pure CV finds 3–5× too many marks and can't type/size them; CAD source doesn't exist for client PDFs — so an LLM reading the drawing is the only mechanism that does the real job. Agentic crop-zoom is most accurate but multi-turn (cannot batch) at ~$1,200–2,000/2,000; a powerful single pass batches at ~$575/2,000 with 24 h turnaround, which is acceptable. Bedrock batch keeps everything on the existing AWS account and avoids a paid Anthropic account, which Summit declined.
**Date:** August 13, 2026

#### MCAA Takeoff Becomes a Separate Module, Not a Rate-Mode Toggle on the Shared Takeoff
**Rule:** MCAA gets its own **MCAA Takeoff** module page — its own configs, Previous Batches, Download/Recalc, and S3 namespace (`mcaa/` prefix on the existing buckets) — fully isolated from the Summit Takeoff. Generic infrastructure (drawing upload, S3 access, config canvas, batch grid) is shared via a `TakeoffProfile` value object (buckets/prefixes/state-machine/processor/rate-service), NOT copy-pasted; Summit's profile defaults to today's exact values so its live path stays byte-identical. The existing Phase-1 MCAA branching in `TakeoffPostProcessor` lifts into the new module's own processor.
**Why:** The May 5 "single-trunk toggle" decision assumed MCAA/Summit divergence would be small and runtime-gateable. It isn't — MCAA diverges at the config (line-body box), the extraction (reads the drawing + properties), the rate source (SQLite keys), and the labor model. A separate module deletes all the accidental complexity of cohabitation (batch typing, cross-type guardrails, "missing = Summit" migration), isolates in-development MCAA from the production Summit path active users depend on, and matches the sunset trajectory (delete a module cleanly instead of untangling flags). Same reasoning already used to keep `MCAARateSheetService` separate from `RateSheetService`. **Supersedes the May 5 "Rate Mode Is a Three-Way In-App Toggle" decision.**
**Date:** August 19, 2026

#### MCAA Extraction Runs On-Demand, Not Bedrock Batch Inference
**Rule:** MCAA drawing extraction uses synchronous, on-demand Bedrock calls (per-drawing, fanned out for concurrency), NOT Bedrock Batch Inference. **Supersedes the batch-delivery half of the Aug 13 decision; the single-pass LLM-vision approach itself stands.**
**Why:** Exec direction (green-lit on-demand): users need a takeoff *now*, not a ~24 h batch turnaround. Measured on-demand cost ~$0.59/drawing at high effort is accepted for immediate results. Opus 4.8's default token-per-minute quota (30M cross-region TPM) is already generous for the target concurrency, so the batch discount isn't needed to make it viable.
**Date:** August 19, 2026

#### MCAA Single-Pass Drawing Extraction Validated (Bench 4/5), With Legend + Tiled High-DPI Input
**Rule:** The connection extractor is a single Opus 4.8 high-effort call per drawing that receives: (1) connection-symbol legend PNGs (`Plans/MCAA-Takeoff/legendFiles/` — 5 panels: BW / field-weld / socket-or-threaded bracket / threaded bar / flanged-boltup incl. half), (2) the rules transcribed as prompt text, and (3) the drawing as a full-page overview PLUS a grid of high-DPI tiles (Bedrock downsamples any image to ~1568px, so a lone full sheet loses the weld marks; tight crops/tiles keep them legible). Returns per-connection type/size/material/quantity/field_weld; stock-length additions computed in code.
**Why:** Bench-tested on the 5 regression drawings: **4/5 exact weld-only** (N2-176304=7, PWP=11, STORMP=29, N2-107418=9; MEOH 11 vs 12 — drawn welds perfect, off by 1 only on stock-length arithmetic). N2-107418's total was right but 2 of its ½″ socket welds read as butt — the known small-bore socket-tick weakness. This replaced a medium-effort / no-legend / full-page run that got every drawing wrong; the legend + tiles + high effort are what fixed it. Cost ~$0.59/drawing on-demand (~$1,170/2,000).
**Date:** August 19, 2026

#### MCAA Extraction Rules (2026-08-19)
**Rule:**
- Extract **all** connection types (BW/SW/THRD/GRV/FLG/BU), not BW/SW-only (supersedes the Round-1 scope lock in `01_master_takeoff_instructions.md`).
- **Weldolet = two branch-size BW** (header attachment + outlet); e.g. a 14×2 weldolet = two 2″ BW.
- **Half bolt-up = quantity 0.5** — a single flange at a spool/off-sheet boundary (mating side dashed or absent); a full bolt-up (both flanges shown solid) = 1.0. A `quantity` field carries this; totals sum fractionally.
- **Field welds** get `Field Weld = "yes"` when the drawing marks them (X, rays, or FW annotation in any color incl. red markup); blank otherwise; welds (BW/SW) only. Per-drawing field-weld count surfaced. Needed for costing Summit's field-crew scope (subcontractor-welded jobs, or shop-builds-spools / field-installs jobs).
- **SW stock-length add = 2 SW welds + 1 coupling** (a socket coupling has two socket welds + is a material item); a BW stock add = 1 BW weld. Stock adds computed in code as `ceil(run/stick)−1` (SS 20 ft, CS 40 ft), only for over-stick runs with no drawn weld.
- **Count run welds only where physically drawn** (a dot/field-weld mark) — never inferred from dimension breaks (dimensions are measurements, not welds).
- **Forced BOM reconciliation** — account for every BOM item's weld ends before finalizing.
**Why:** Each rule came from a real failure or requirement surfaced reviewing the bench output against real Lilly drawings (S130, MEOH).
**Date:** August 19, 2026

#### MCAA BOM Property Extraction: Single-Pass With the Per-Component Allowlist Injected
**Rule:** Property extraction (for composing rate keys) starts as a single AI pass that outputs every field per BOM line **with the `ComponentProps` per-component allowlist injected into the prompt**, so the model only fills properties that apply to each component. Every composed key is validated against `cdx_weblem_rates.db` (does it resolve?); misses flag for review, never silently drop. Upgrade specific components to a two-stage (identify-then-extract-applicable-props) approach only if evidence shows they drift.
**Why:** Two-stage is more accurate but heavier; single-pass with the allowlist captures most of the accuracy at single-pass simplicity, and we're in "wire it up then refine on evidence" mode. Key misses are silent, so the validation layer is mandatory.
**Date:** August 19, 2026

#### New MCAA AWS Objects Use `mcaa-takeoff-*` Names
**Rule:** Every net-new AWS object for the MCAA takeoff carries an intuitive `mcaa-takeoff-*` name mirroring the `summit-takeoff-*` objects (`mcaa-takeoff-extract` Lambda, `mcaa-takeoff-aggregate`, `mcaa-takeoff-orchestrator` state machine, ECR `mcaa-takeoff-extract`, `mcaa-takeoff-*` IAM roles/policies). S3 stays on the existing buckets with an `mcaa/` key prefix.
**Why:** Steve or the director scanning the AWS account should recognize at a glance what each object is for and that it's separate from Summit. Prefix-in-existing-buckets avoids new bucket/IAM provisioning.
**Date:** August 19, 2026
...
**Rule:** `lookup_key` concatenates its segments in a fixed order via a plain `TEXTJOIN`, blanks skipped: `NewComp`, `Reducing`, `NewMaterial`, `Merged_Props`, `pressure_rating`, `class_rating`, `schedule`, `weight_class`, `length`, `connection_type`, then `size_1…size_7`. `connection_type` sits immediately **before the sizes**; its multiple ends are comma-joined in stored order (`BW,BU`). `connection_qty` is **NOT** part of the key — it is redundant with the per-end `connection_type` token list; the column is retained only for validation / AI-reference use. Sizes and connection tokens emit in **stored order** — the rate-sheet row's stored order is canonical, and the C# takeoff side must emit each item's per-end pairs in that same order for a byte-identical match. `Merged_Props` is the only sorted segment (properties sort A→Z). `length` carries an explicit uppercase unit (`FT`/`IN`), which C# must also emit. Reducing flanges store `BU` first because `BU` pairs with the larger size (`size_1`). The full recipe, Excel formulas, and column map live in `Plans/MCAA_Key_Composition.md` (canonical).
**Why:** Two reasons the key is shaped this way. (1) An earlier contract sorted the (size, connection) pairs size-descending with a connection-A→Z tie-break; sorting canonicalized away order that carries meaning (run vs. branch, large vs. small end), collapsing genuinely-different items into ~1,400 false-duplicate keys (a `2×1` reducer keyed identically to `1×2`). Stored order on both sides preserves those distinctions and still yields identical keys. (2) `connection_qty` was dropped from the key once connections are listed one token per end — the count is then implicit, so keying on it adds no discrimination (two rows with the same token list necessarily share qty). Verified lossless: qty equals the token count on all 96,584 keyed rows, with zero rows where qty carried information the tokens didn't.
**Date:** 2026-08-06
