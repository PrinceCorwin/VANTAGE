...
- **Consolidate Schedule legend:** Merged two gold/yellow legend items ("Forecast ≠ P6" and "P6 vs Vtg Discrepancies") into single "Vtg ≠ P6" legend entry.
...
- **Feedback Board email notifications:** Admins receive email when users submit new feedback items. Users receive email when an admin changes their feedback status (New → In Progress → Complete/Rejected). Fixed bug where admin notification query used wrong table names (Users/Admins instead of VMS_Users/VMS_Admins).
