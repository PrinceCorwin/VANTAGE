...
### April 18, 2026 (Assets Folder Reorganization, VP vs Vtg Prep Dialog)
- **Assets folder reorganization:** Consolidated image assets into a single top-level `Assets/Images/` tree with three purpose-named subfolders — `System/` (7 app icons/logos: AppIcon.ico, CoverPic.png, MilestoneLogoPngTransparentBG.png, SummitLogoNoText.jpg, SummitS-Full Summit Peak Logo.jpg, summit logo.svg, summitLogo_largeText.png), `HelpSidebar/` (30 help manual screenshots), and `Dialogs/` (new — for one-off dialog images like `vp-vtg-prep.png`). Previously images lived in two parallel top-level folders (`Images/` and `Help/*.png`).
...
- **csproj cleanup — Help PNG entries collapsed to a single glob:** Replaced 25 `<None Remove>` entries, one `<None Update="Help\*.png">` block, and 29 individual `<Content Include="Help\xxx.png">` entries with a single glob `<Content Include="Assets\Images\HelpSidebar\*.png" Link="Help\%(Filename)%(Extension)">`. The `Link` attribute preserves the runtime output layout (`bin/Debug/net8.0-windows/Help/*.png`) so the WebView2 virtual host mapping (`help.local` → `{baseDir}/Help`) and manual.html's sibling-relative `<img src>` tags continue to work unchanged. Net ~80 lines shorter and self-maintaining for future screenshots.
...
- **Help manual updated:** VP vs Vtg Report entry in the Tools menu section now documents the prep dialog, annotated screenshot, and the Do-not-show-again persistence.
- **Key files:** `Assets/Images/` (new tree), `Dialogs/VPvsVtgPrepDialog.xaml/.cs` (new), `MainWindow.xaml.cs`, `VANTAGE.csproj`, `VANTAGE.Installer/VANTAGE.Installer.csproj`, `Help/manual.html`, `Plans/Project_Status.md`, plus the XAML/C# reference updates listed above.
