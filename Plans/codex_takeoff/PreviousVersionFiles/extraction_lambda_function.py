...
def lambda_handler(event, context):
    try:
        ...
        # Guard fires in both batch and non-batch (direct-invoke) modes. In batch mode the
        # marker lands in the per-batch failures/ prefix and the aggregation Lambda picks it
        # up. In non-batch mode (manual debug invocation) the marker lands at the top-level
        # failures/ prefix parallel to the success path's extractions/ prefix so a zero-item
        # response can't masquerade as success.
        ...
        if bom_row_count == 0:
            ...
            if batch_id:
                ...
                failure_key = f"failures/{drawing_name}.json"
...
def parse_extraction_response(raw_text):
    """Parse JSON from Claude's response, tolerating markdown fences, preamble text, and trailing commentary."""
    text = raw_text.strip()

    # Find the outer boundaries of the JSON object. Claude 4.6 sometimes emits a
    # chain-of-thought preamble before the JSON and may add commentary after it.
    # Slicing from the first `{` to the last `}` strips both cleanly.
    first_brace = text.find("{")
    last_brace = text.rfind("}")

    if first_brace == -1 or last_brace == -1 or last_brace < first_brace:
        logger.error(f"No JSON object found in Claude response")
        logger.error(f"Raw response (first 2000 chars): {raw_text[:2000]}")
        raise ValueError("Claude did not return any JSON object in the response")
