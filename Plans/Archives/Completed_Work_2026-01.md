# VANTAGE: Milestone - Completed Work (January 2026)
...
- **Token bug fix in WorkPackageView.xaml.cs:**
  - Fixed `{PrintDate}` â†’ `{PrintedDate}` in predefined TOC items (didn't match TokenResolver)
  - Fixed `{ScheduleActivityNo}` â†’ `{SchedActNO}` in predefined TOC items (didn't match TokenResolver)
...
- **Help Sidebar polish and content updates:**
  - Renamed "Help / AI Sidebar" â†’ "Help Sidebar" in MainWindow.xaml menu items (2 locations)
  ...
  - Added WebView2 virtual host mapping in SidePanelView.xaml.cs (`help.local` â†’ Help folder) to enable local image loading
...
- **Progress Log Performance Optimization:**
  ...
  - Delete performance: 18 min â†’ 17 sec for 57K records
  - Replaced client-side SqlBulkCopy upload with server-side INSERT...SELECT (data never leaves Azure)
  - Upload performance: 157 sec â†’ 37 sec for 57K records
  - Fixed PercentEntry conversion (0-100 â†’ 0-1 decimal) for Val_Perc_Complete column
  ...
  - Renamed Admin menu "Edit Snapshots" â†’ "Manage Snapshots"
...
- **Progress Log Upload Feature (Admin > Manage Snapshots):**
  ...
  - Uses VMS_ColumnMappings table for dynamic column mapping (ColumnName â†’ AzureName)
  - Calculates fields at upload time: Status (from PercentEntry), EarnMHsCalc (BudgetMHs Ã— PercentEntry / 100), ClientEquivEarnQTY
...
- **Excel Import/Export - LineNumber Column Fix:**
  - Fixed typo in ExcelExporter.cs: `Tag_LineNumber` â†’ `Tag_LineNo` (line 43)
...
- **AI Progress Scan - OCR Improvements and UI Enhancements:**
  ...
  - Added "00" â†’ "100" OCR heuristic to handle missed leading 1 in handwritten entries
...
- **EarnQtyEntry Recalculation Bug Fixed:**
  ...
  - Fixes: PercentEntry â†” EarnQtyEntry sync, Quantity changes, BudgetMHs changes
...
- Help Sidebar - Search field improvements:
  - Added clear button (âœ•) that appears when text is present
...
- **Activity Import date/percent cleanup** - During import, cleans up date/percent inconsistencies:
  - PercentEntry = 0 â†’ clears both SchStart and SchFinish
  - PercentEntry > 0 with no SchStart â†’ sets SchStart to today
  - SchStart in future â†’ clamps to today
  - PercentEntry < 100 â†’ clears SchFinish
  - PercentEntry = 100 with no SchFinish â†’ sets SchFinish to today
  - SchFinish in future â†’ clamps to today
...
- **Schedule Change Log feature** - Apply Schedule detail grid edits to live Activities:
  ...
  - ScheduleChangeLogDialog accessible via Tools â†’ Schedule Change Log
...
- Help Sidebar - Search functionality:
  - Added search field below context header with Ë„/Ë… navigation buttons and match counter
...
- Work Package Module - Drawings integration (Phase 1):
  ...
  - Local folder fetch with smart matching: full DwgNO â†’ fallback to last two segments (e.g., "017004-01")
...
- Work Package Module - Grid editor improvements:
  - Added Edit button (âœŽ) for columns with edit panel (name + width fields)
...
- Work Package Module - Form (Checklist) editor improvements:
  - Added Edit buttons (âœŽ) for columns, sections, and section items
