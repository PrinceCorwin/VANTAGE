...
### MCAA Proxy Mappings — Discovered During Parity-Test Scoping (2026-04-29)
...
- **Out-of-scope items (not in WebLEM `PIPING SYSTEMS` root).** The SkySkraper scrape covers 1,671 leaves under `PIPING SYSTEMS` only. WebLEM's `dom_leaves.json` shows 1,926 total leaves across 10 top-level sections — 255 leaves under `Instrumentation`, `HVAC Equipment`, `Plumbing Equipment`, `Plumbing Fixtures`, `Refrigeration Equipment`, `Treatment Plant Equipment`, `Hangers/Sleeves/Inserts`, `Excavation & Backfill`, and `Miscellaneous Labor Operations` were never fetched. Practical impact for the parity test: valve actuators (`ACT` component, e.g. `LP1YFV3021AC` rack-and-pinion) live under `Instrumentation > Field Mounted Instruments & Devices` — no MCAA-piping equivalent. Three options when these surface: fall back to Summit RateSheet for the line, roll into parent valve install, or treat as out-of-scope and exclude both totals from comparison.
