...
**Last Updated:** June 24, 2026 (pull-guard + trigger fix + UOM required + MyRecordsOnly cleanup shipped)
...
### Critical
- **Existing-drift cleanup from past trigger-disable windows.** Before 2026-06-24, every bulk push disabled `TR_VMS_Activities_SyncVersion` database-wide for the upload window. Any other user's UPDATE that landed during one of those windows didn't bump `SyncVersion`, so every other local cache is silently behind on those rows. No way to know which specific rows are affected without row-by-row content comparison. Two cleanup options: (a) one-time SQL bump that increments `SyncVersion` on every row in `VMS_Activities` — every user's next sync re-pulls everything; or (b) Tools menu / admin nudge to have every user do a manual reset-and-re-pull. Either path makes the silent-drift backlog visible exactly once.
- **Push re-claim of reassigned rows.** `SyncManager.cs:184` lists `AssignedTo` in `syncColumns`, so every push UPDATE writes whatever the local copy has for that field. If a user edits a row at 9am, an admin reassigns it to another user at 10am, and the original user syncs at 11am, the push silently overwrites the reassignment and the row becomes theirs again. Adjacent to the MyRecordsOnly cleanup fix shipped 2026-06-24 (same root cause: app trusts local state for ownership). Fix needs either a compare-and-set on `AssignedTo` at push time, or excluding `AssignedTo` from push unless the user explicitly changed it.
- **UOM cleanup for existing blank rows.** Audit 2026-06-24: 562,573 rows across 10 users / 9 projects have blank UOM. With UOM now required (shipped 2026-06-24), every affected owner sees their metadata-error badge tick up by their share, and Submit Week blocks for any project with any blank-UOM row. CompType doesn't drive UOM consistently (varies per project, plus the column used for GSKT/etc differs per project), so no global backfill is practical. Per-project cleanup is the path: work with each project owner to identify whichever column drives UOM in that project's data and write a targeted UPDATE. Nine projects, nine SSMS runs. Until done, affected users live with elevated badge counts and have to fill in UOM as they touch rows.

### High Priority
- **Azure performance fixes (ProgressLog + Submit Week) — remaining off-hours work.** (1) `CommandTimeout = 0` on ProgressLog upload — **DONE 2026-04-21** (tiered: 3600s for big INSERT/UPDATE/DELETE, 120s for aggregates, 60s for single-row inserts; in `AdminSnapshotsDialog.xaml.cs`). (2) `ExportedBy` INCLUDE on `IX_ProgressSnapshots_Group_Lookup` — **DONE 2026-04-21** via SSMS. (3) **Quota increased 2026-06-24 — clustered rebuild can now proceed.** `VANTAGE_global_ProgressLog` clustered index — attempted 2026-04-21, failed at 13% because the table alone was 15.9 GB in a 30 GB DB (shared with pts_*, deliveryPlanner_* apps) and a HEAP→clustered rebuild needs ~2x the table size in scratch space. Director secured the Azure quota bump 2026-06-24. Schedule a maintenance window (after-hours, ideally weekend) to retry the rebuild with `ONLINE = ON, MAXDOP = 1` if possible to keep app writes flowing; otherwise plan for a downtime window. Verify post-rebuild: fragmentation drops to <5%, ProgressLog reads (`ManageProgressLogDialog` refresh) and admin upload queries return in <2s. (4) `VMS_ProgressSnapshots` clustered PK rebuild — **DONE 2026-04-21** (was 58.7% fragmented / 765 MB; rebuilt ONLINE with FILLFACTOR=90, now 0.01% / 402 MB).
