...
- **Takeoff Post-Processing Pipeline** — All operate on the downloaded Excel, no AWS changes needed. See `summit-takeoff-integration-guide.md` for details.
  ...
  2. **Rate application** — See `Rate_Application_Plan.md` for full mapping table, decisions, and implementation plan. Core implementation complete (RateSheetService, ApplyRates, Missed Rates tab). Admin email notification for missed data complete. TODO:
