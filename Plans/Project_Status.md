...
- **FEATURE:** Find/Replace blank values in Progress View — Current Find/Replace dialog cannot target empty/blank field values. Need option to find blanks and replace with a specified value (useful for bulk-filling empty metadata fields)
- **BUG:** Snapshot submit aborts completely on conflicts — When submitting snapshots, if any records were already submitted by another user or into another project, the entire submit operation aborts. Should skip conflicting records and continue with valid ones, then report which were skipped
- **FEATURE:** Import From P6 project list from snapshots — Currently the project list in Import From P6 dialog is derived from user's local Activities table. Should query user's submitted snapshots instead. Add message if no snapshots exist ("Submit Week needs to be done first"). Add footer message: "If desired project not visible, cancel and submit week first"
