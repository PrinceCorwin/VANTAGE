...
- **SkySkraper pending TODO — patch 4 generator scripts to emit short workbook paths (load-bearing).** After the 2026-05-18 path-shortening pass, `scripts/build_workbooks.py`, `scripts/cdx_build_workbooks.py`, `scripts/cdx_build_rates_review.py`, and `scripts/cdx_create_review_samples.py` still produce the legacy long names. Any regeneration today recreates pre-MAX_PATH paths and may fail to find existing short-named files (notably the 9 hardcoded long source paths in `cdx_create_review_samples.py` that no longer resolve on disk). Lift the regex chain verbatim from `scripts/shorten_workbook_paths.py` (`shorten(name)`). Full substitution table with ordering rules in `SkySkraper/SynologyDrive/Plans/cdx_Project_Status.md`, May 18 section. Before running ANY bulk regen after this lands: set the SDrive sync task to "Sync everything to this computer" first (see Completed_Work May 19 entry on zombie cleanup).
- **SkySkraper pending TODO — delete throwaway scripts after generators patched and verified clean.** `scripts/shorten_workbook_paths.py`, `scripts/diagnose_zombies.py`, `scripts/sample_zombie_compare.py`, `scripts/delete_zombies.py`. Keep them as a safety net until all three PCs (work + both Legions) confirm 0 zombies/0 orphans after the next bulk rename.
...
- **Takeoff initial-download filename inconsistency** — The auto-download immediately after a takeoff completes saves with a `takeoff_` prefix (e.g., `takeoff_AwsTO-20260510-...xlsx`) because `TakeoffView.xaml.cs:334` falls back to `$"takeoff_{batchId}.xlsx"` when batchName is null, and the auto-download path (`TakeoffView.xaml.cs:111` / `:302`) never passes one. Previous Batches → Download Excel and Recalc Excel both produce clean names without the prefix, so users see filename inconsistency between the same logical file across paths. Fix: drop `takeoff_` from the fallback string so all paths converge on `{batchName}.xlsx` or `{batchId}.xlsx`. Local-filename only — no S3 or Lambda impact.
