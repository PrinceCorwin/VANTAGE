...
- **Reset User Settings dialog** — Add Settings menu item "Reset User Settings" that opens a dialog listing all user settings in natural language with checkboxes. User picks which settings to reset; the selected rows are deleted from `UserSettings` (SettingsManager.RemoveUserSetting). Natural-language labels mapped per setting name (e.g., `SkipVPvsVtgPrepDialog` → "Show VP vs Vtg prep dialog again"). Covers `AnalysisGroupField`, `AnalysisCurrentUserOnly`, `AnalysisSelectedProjects`, grid layouts, UDF mappings, theme, and any future per-feature flags.
