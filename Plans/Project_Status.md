...
## Ready to Publish
- **Snapshots — Export selected snapshots to Excel** — both the user-facing Manage My Snapshots dialog (File menu) and the Admin Manage Snapshots dialog gained an **Export** button that writes the selected snapshot group(s)' detail records to an `.xlsx` (one row per record, all ~83 data columns, plus `AssignedTo`/`WeekEndDate` context columns). New shared `Utilities/SnapshotExcelExporter.cs` (ClosedXML, formula-injection guarded, 3-place rounding, cached reflection). Export is read-only so it is not ownership-gated. Admin dialog widened 700→820 for the extra button. **Screenshot TODO:** the Manage My Snapshots manual figure (`progress-manage-snapshots.png`) now shows an extra button — re-capture when convenient.
...
- **⚠️ BLOCKED — needs Azure login credentials from the director** before any storage-account setup can proceed. Shelved 2026-07-19 pending those creds. (Related: the "Migrate Email Service to company Azure account" backlog item also depends on company-Azure access — coordinate; if the director's access is being (re)granted, knock out both.) Next step once in: open Azure Portal → Storage accounts, find or create a storage account, create a `tutorials` container, upload a test MP4, and confirm the blob URL plays in a browser.
...
- **Help manual screenshots audit (IN PROGRESS)** — Reviewing every section of `Help/manual.html`, updating outdated screenshots and adding missing ones. **Workflow (working well):** Steve pastes screenshots in a Claude Code session; Claude saves each into `Assets/Images/{System,Sidebar,Dialogs}/` and wires the `<img>` into the right section.
  ...
  - **Still REMAINING to capture/update:**
    - Progress Books — update full-view to v26.3.3; multi-select Value picker open (checkboxes + Select all/Clear); bulk-export completion summary; verify editor/preview/AI-scan shots
    - Work Packages — update module shots; WP Templates tab File Name Pattern fields with live `Example:` lines; bulk-export `ProgBooks - [timestamp]` output structure
    - AI Takeoff — entire module, zero images (overview, configs, unit rates, processing, previous batches, Excel output, import dialog)
