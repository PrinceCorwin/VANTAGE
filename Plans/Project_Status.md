...
### AI Takeoff — Material Grade (`matl_grade`) Extraction
- **Goal:** New Material tab column extracting specialty material grades (chrome-moly steels like P11/P22, plastic SDR ratings, copper temper, high-purity / sanitary / food-grade designations, etc.) from BOM descriptions. Populated for both Summit and MCAA takeoffs — Summit ratesheet ignores the value, MCAA rate lookup will use it (Phase 2). Material-tab consumers (expediters, buyers) benefit immediately.
- **Shape decision (2026-05-09):** Fold `matl_grade` into the existing `MatRefTable.xlsx` as additional columns (`Grade Description`, `Grade Abbreviation`), denormalized one row per (material, grade) pair. Materials without specialty grades have blank grade columns. The "grade is the material" case (description references only the grade with no separate material indicator) becomes implicit because the row containing the grade also names the matl_grp. Single source of truth, no separate ref table needed. The earlier separate-`MatGradeRefTable.xlsx`/`BodyTypeRefTable.xlsx` plan is retired.
- **Pending (next session):** receive user's consolidated material + grade pairs spreadsheet → upload to `s3://summit-takeoff-config/MatRefTable.xlsx` → update `summit-takeoff-poc/lambda_function.py:load_material_reference_table` from 3 columns to 5 → update `extraction_prompt.txt` (new `matl_grade` field bullet, integrated Material Group Matching Rule covering grade extraction, OUTPUT FORMAT JSON schema entry) → update `aggregate-deploy/lambda_function.py:fixed_columns` to add `matl_grade` between `class_rating` and `length` → Pattern A+B+C deploy → parity test against the 3-drawing baseline. Working notes: `~/.claude/plans/i-have-no-problem-polymorphic-star.md`.
- **Edge cases the user will decide:** brazing alloys (`SILFOS`, `SILVER`) and `Silicon` — include under a base material in `MatRefTable`, omit (consumables aren't pipe materials), or carry a separate field.
- **Deferred from this scope:** `body_type` extraction (concentric/eccentric/male/female/etc.) — needs its own values list which isn't ready. Revisit as a separate effort later.
