...
- **Help manual screenshots audit** — Review all sections of `Help/manual.html`, update outdated screenshots to reflect current UI, and add missing screenshots for features that have none (e.g., ActNO Split Ownership Check dialog, Sync Incomplete warning, any other recently added dialogs or UI changes). **Workflow:** Steve posts screenshots in a Claude Code session; Claude copies each into `Assets/Images/{System,Sidebar,Dialogs}/` and wires the `<img>` into the right section (proposed styling: a shared theme-aware `.manual-shot` CSS class — rounded corners + soft shadow — instead of repeating inline styles). **Specific new/changed sections still needing capture (added 2026-07-18, currently text-only — Claude failed to flag these at the time):**
  - Progress Books — multi-select Value picker (checkboxes + Select all/Clear) and the bulk-export completion summary
  - Work Packages — WP Templates tab: the two File Name Pattern fields with their live `Example:` lines
  - Work Packages — bulk-export folder / `ProgBooks - [timestamp]` output (Progress Books) if illustrating output structure
  - Admin → Edit Projects — the new filterable/sortable `SfDataGrid` with a column filter dropdown open
  - Progress — global search tooltip/behavior (optional; text change only)
