...
### High Priority
- **Add `AssignedTo` column to `VANTAGE_global_ProgressLog`** — Requires DBA permission (table has 14.9M rows). Currently the upload loses the original record owner: `UserID` is the admin who ran the upload (hardcoded in `AdminSnapshotsDialog.UploadSnapshotsToProgressLog` line 399-401), and `VMS_ColumnMappings` has two broken rows (`AssignedTo` with no `AzureName`, and an orphan row with `AzureName=UserID` and no `ColumnName`) — neither row does anything because the mapping loader requires both fields non-empty. Result: when multiple users upload snapshots for the same ProjectID at the same upload Timestamp, their rows are indistinguishable in ProgressLog (observed case 2026-04-18: Steve and Grant.Gilbert both uploaded 24.005. records at 21:41:27; no way to extract just Grant's 33,744 rows from ProgressLog alone). Fix, once DBA approval secured: (1) `ALTER TABLE VANTAGE_global_ProgressLog ADD AssignedTo NVARCHAR(100) NULL;` — metadata-only operation in SQL Server, non-blocking. (2) `UPDATE VMS_ColumnMappings SET AzureName = 'AssignedTo' WHERE ColumnName = 'AssignedTo';` (3) `DELETE FROM VMS_ColumnMappings WHERE (ColumnName IS NULL OR ColumnName = '') AND AzureName = 'UserID';`. No application code change needed — existing mapping loop picks it up automatically. Existing 14.9M rows stay NULL (unfixable without the original snapshots, most of which have been purged past 21 days); future uploads populate correctly. Workaround until then: extract from `VMS_ProgressSnapshots` directly while the 21-day snapshot retention still has the records.
- **Azure performance fixes (ProgressLog + Submit Week)** — Investigation on 2026-04-18 found: (1) The ProgressLog upload dialog uses `CommandTimeout = 0` (infinite) in four places in `Dialogs/AdminSnapshotsDialog.xaml.cs` (lines 402, 429, 455, 482). When a TCP connection drops mid-operation the client hangs forever. Today's 178,193-row upload committed all data on Azure (verified via row count + `Apr 18 2026 9:41PM cnt=178193` on `VANTAGE_global_ProgressLog`) but the UI was wedged because the final `UPDATE VMS_Activities SET AzureUploadUtcDate` (line 496-504) triggered `TR_VMS_Activities_SyncVersion` on 178k rows and the response was lost on a dropped socket. Set to tiered values: 1800s on main INSERT and final UPDATE, 60s on RespParty SELECT and tracking INSERT. (2) Submit Week timed out on `SELECT TOP 1 ExportedBy FROM VMS_ProgressSnapshots ...` — the existing `IX_ProgressSnapshots_Group_Lookup` has correct keys `(AssignedTo, ProjectID, WeekEndDate)` but does NOT include `ExportedBy`, forcing key lookups into the clustered PK which is 69% fragmented. Add `ExportedBy` to the INCLUDE list via `CREATE NONCLUSTERED INDEX ... WITH (DROP_EXISTING = ON, ONLINE = ON)`. (3) `VANTAGE_global_ProgressLog` is a HEAP with 14.9M rows (only one nonclustered index on `Tag_ProjectID, UDF18, Timestamp, Val_TimeStamp`). Add `CREATE CLUSTERED INDEX CX_ProgressLog_Timestamp ON VANTAGE_global_ProgressLog ([Timestamp]) WITH (ONLINE = ON, DATA_COMPRESSION = PAGE)`. Off-hours. (4) `VMS_ProgressSnapshots` clustered PK is 69% fragmented — `ALTER INDEX PK_... REBUILD WITH (ONLINE = ON)`. Off-hours, same window as (3). Priority order: timeouts → index A → indexes B/C in a maintenance window.
- **Remove RespParty breakdown from ProgressLog uploads** — Legacy behavior from OldVantage. The upload in `Dialogs/AdminSnapshotsDialog.xaml.cs:419-476` runs a per-group `SELECT RespParty, COUNT(*) FROM VMS_ProgressSnapshots GROUP BY RespParty` and writes one tracking row per RespParty into `VMS_ProgressLogUploads`. We no longer need ProgressLog uploads broken out by Responsible Party. Simplify to one tracking row per uploaded group (Username + ProjectID + WeekEndDate), drop the RespParty column from tracking (or default it), and delete the per-RespParty loop. Check `ManageProgressLogDialog.xaml.cs` (REFRESH scan + DELETE path) and the grid column config — they'll need adjustment to match the new tracking shape.
