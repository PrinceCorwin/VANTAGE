...
**Last Updated:** June 23, 2026 (Critical: pull-guard hole in SyncManager + UOM to required metadata, ordered)
...
### Critical
These two items are ordered. **(1) MUST ship before (2).** Investigation 2026-06-23 surfaced a pre-existing pull-overwrite hole that has to be closed before UOM becomes required, otherwise adding UOM would expand a known silent-data-loss surface rather than just gating new validation.

1. **Fix the pull-guard hole in `SyncManager.PullRecordsAsync` (independent of UOM, fixes today's date + 9-field rules too).** `Utilities/SyncManager.cs:710-873` pulls Azure rows with `INSERT OR REPLACE INTO Activities ...` (line 821) and unconditionally forces `LocalDirty = 0` (line 844). There is NO `WHERE LocalDirty = 0` guard. A row blocked by the partial-push gate (`PushRecordsAsync:132-172`) stays `LocalDirty = 1` locally but gets silently clobbered on the next pull if Azure's `SyncVersion` for that row has bumped — e.g. another user pushed an edit, an admin reassigned, ProgressLog upload bumped versions — or on any full re-pull (toggling "My Records Only" off resets `LastPulledSyncVersion`). This contradicts the "partial sync is safe" invariant currently stated in `CLAUDE.md:191` and the corresponding `Plans/Decisions.md` entry; the invariant is aspirational, not enforced. **Fix:** replace the bulk INSERT OR REPLACE with an upsert that skips locally-dirty rows — `INSERT INTO Activities (...) VALUES (...) ON CONFLICT(UniqueID) DO UPDATE SET <cols> WHERE Activities.LocalDirty = 0`. New rows insert normally; clean local rows update from Azure (today's behavior); dirty local rows are preserved. Add a `SkippedDirtyConflicts` list to `SyncResult` so the sync dialog can surface "Azure has newer data for N of your unsynced rows — they were not overwritten" (read-only signal; last-write-wins on the eventual push, which is a separate compare-and-set design out of scope here). After this fix, update the `CLAUDE.md:191` invariant + the `Plans/Decisions.md` entry to match reality. Verify: (a) blocked-dirty row stays put when an unrelated Azure update bumps its SyncVersion; (b) full re-pull (My Records Only toggle off) preserves dirty rows; (c) new Azure rows still flow down; (d) IsDeleted=1 records in Azure still delete the local row even when LocalDirty=1 (open question — current code deletes unconditionally on line 800; decide whether that's correct or whether tombstone should also defer).
2. **Add `UOM` to required metadata (depends on item 1).** `Utilities/ActivityValidator.cs:13-17` → `ActivityRequiredMetadata.Fields` currently lists 9 fields (`ProjectID, WorkPackage, PhaseCode, CompType, PhaseCategory, SchedActNO, Description, ROCStep, RespParty`) — UOM is not among them. Add `"UOM"` to the `Fields` array and the change propagates via `FieldsDisplay` and `BuildMissingFieldSql(alias)` to every consumer. Code surface is one array entry — verified call sites are all data-driven from `Fields`: `SyncManager.PushRecordsAsync` (per-row partial gate), `ProgressView.xaml.cs:322,395,3401,4326,5861,5878` (sync-gate SQL + Submit Week + reassign), `ImportTakeoffDialog.xaml.cs:68` + `ImportTakeoffDialog.xaml:319-358` (the metadata-mapping section is an ItemsControl bound to `BuildMetadataItems()` and auto-grows from 9 to 10 rows — no XAML change), `AuditAllRecordsDialog`, `ValidateMyRecordsDialog`, `ModifySnapshotDialog`. Schema is already `UOM TEXT NOT NULL DEFAULT ''` end-to-end (`DatabaseSetup.cs:357`, `SyncManager.cs:200` includes UOM in syncColumns, snapshot table has it too) — no migration. **Open: existing-data remediation.** Live Azure `VMS_Activities` will have rows with blank/NULL UOM; once the gate goes in, they block their owners' sync (per-row) and Submit Week (project-wide). Options to choose from before shipping: (a) pre-deploy audit + targeted user callout, no backfill; (b) pre-deploy admin backfill to a default UOM (e.g. 'EA' or 'UNK') — risk: silently rewrites data; (c) grandfather clause in `ActivityValidator` keyed on `CreatedUtcDate < cutoff` — leaves permanent special-case logic; (d) phased rollout: warning-only release first, hard-block release later. User has not decided — discuss before implementation. **Also update on landing:** `Help/manual.html` lines 639, 967, 976, 1018 (the "9 required-metadata fields" prose), `Plans/Decisions.md:59-62` (the canonical-source-of-truth entry referencing "9 fields"), `Plans/PRD_NewActNOsFromP6.md:48` ("Nine required fields total" placeholder list), and the `Plans/Decisions.md:756` Validate My Records description. `CLAUDE.md:189` ("9 required-metadata field names") also needs the bump.

### High Priority
- **Azure performance fixes (ProgressLog + Submit Week) — remaining off-hours work.** (1) `CommandTimeout = 0` on ProgressLog upload — **DONE 2026-04-21** (tiered: 3600s for big INSERT/UPDATE/DELETE, 120s for aggregates, 60s for single-row inserts; in `AdminSnapshotsDialog.xaml.cs`). (2) `ExportedBy` INCLUDE on `IX_ProgressSnapshots_Group_Lookup` — **DONE 2026-04-21** via SSMS. (3) **Quota-increase request planned 2026-06-22 (Mon):** `VANTAGE_global_ProgressLog` clustered index — attempted 2026-04-21, failed at 13% because the table alone is 15.9 GB in a 30 GB DB (shared with pts_*, deliveryPlanner_* apps) and a HEAP→clustered rebuild needs ~2x the table size in scratch space. Director has regained access to the company Azure account; quota-increase request goes in Monday 2026-06-22. Once granted, retry the clustered rebuild. Fallback if we need the scan speedup sooner: add a nonclustered index on `[Timestamp]` instead (~300–500 MB, fits in current headroom) — script ready. (4) `VMS_ProgressSnapshots` clustered PK rebuild — **DONE 2026-04-21** (was 58.7% fragmented / 765 MB; rebuilt ONLINE with FILLFACTOR=90, now 0.01% / 402 MB).
