...
- **SkySkraper TODO — delete throwaway path-shortening / zombie-cleanup scripts on user direction.** `scripts/shorten_workbook_paths.py`, `scripts/diagnose_zombies.py`, `scripts/sample_zombie_compare.py`, `scripts/delete_zombies.py`. The original "safety net" condition is satisfied as of 2026-05-19: work PC + new personal Legion both at 0 zombies / 0 orphans, old Legion permanently uninstalled from SDrive, so the resurrection-risk surface is closed. Scripts remain in place per "ask first" rule until user directs deletion. Not load-bearing; the generators now apply the substitution rules at write time via `scripts/path_shortening.py`, so no future cleanup pass is expected to need them.
