...
**Last Updated:** June 25, 2026 (Hotfix: reassign SyncVersion+1 regression fixed; DeletedRecords timeout + filtered index + async refresh; pull plan adaptive — clustered scan for bulk pulls)
...
### Critical
- **Push re-claim of reassigned rows — narrow race only.** Push DOES gate on ownership: `SyncManager.cs:281-296` reads Azure's `AssignedTo` into `ownershipMap`, then the filter drops any row where local's `AssignedTo` doesn't match, returning "No longer assigned to you" in the sync result. The common-case sequence (user edits 9am, admin reassigns 10am, user syncs 11am) is safely rejected — no silent overwrite. The bug is a narrow race within a single push call: between the Step 3 ownership SELECT (`:267-279`) and the Step 3 staging UPDATE (`:354-363`), an admin reassign that lands in the gap (~1–3s for a typical push, 10–30s for a 50K-row sync) gets silently overwritten because the staging UPDATE writes local's `AssignedTo`. Fix shapes: (a) wrap Steps 2/3 in a SERIALIZABLE transaction so range locks on the joined UniqueIDs block concurrent admin UPDATEs until commit — lock-pile-up is invisible for everyday sync (disjoint UniqueIDs per user) and bounded for admin bulk reassigns / ProgressLog uploads; or (b) add `OriginalAssignedTo` to `#UpdateStaging` plus `AND a.AssignedTo = s.OriginalAssignedTo` on the staging UPDATE JOIN (compare-and-set, no broader locking).
